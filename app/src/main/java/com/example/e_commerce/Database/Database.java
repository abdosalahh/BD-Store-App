...
//            (id "name "image "price "selled "quantity "cate_id integer
    ...
    public Product get_products_by_id(int id) {
        database = getReadableDatabase();
...
//            (id "name "image "price "selled "quantity "cate_id integer
    ...
    public void update_quantity_in_stock(int id, int quantity) {
        SQLiteDatabase database1 = getWritableDatabase();
        int new_quantity;
        Cursor cursor = database1.rawQuery("SELECT * FROM  product WHERE  id = '" + id + "'", null);
        cursor.moveToFirst();
        new_quantity = cursor.getInt(5);
        new_quantity -= quantity;
        ContentValues values = new ContentValues();
        values.put("quantity", new_quantity);
        database1.update("product ", values, "id = '" + id + "'", null);
        database1.close();
    }
    ...
    {
        database = getReadableDatabase();
        Cursor cursor = database.rawQuery("select * from product where cate_id = '" + id + "'", null);
        ArrayList<Product> arrayList = new ArrayList<>();
...
//            (id "name "image "price "selled "quantity "cate_id integer
    ...
    public void confirm_order(Order order, ArrayList<Cart> carts) {
        ...
        for (int i = 0; i < carts.size(); i++) {
            ...
            update_quantity_in_stock(carts.get(i).getProduct_id(), carts.get(i).getQuantity());
            database2.close();
        }
    ...
    public ArrayList<Product> get_product_ordered_chart() {
        database = getReadableDatabase();
        Cursor cursor = database.rawQuery("SELECT * FROM product ORDER BY selled DESC", null);
        ArrayList<Product> arrayList = new ArrayList<>();
